...
def _get_mamba_class():
    try:
        ...
        raise ImportError(
            "The Mamba decoder requires mamba-ssm. Use decoder_type='lstm' or "
            "decoder_type='transformer' if mamba-ssm is unavailable."
        ) from exc
    return Mamba
...
class HCMA_2DCNN(nn.Module):
    """2D-CNN encoder with Mamba, LSTM, or Transformer temporal decoder."""
    ...
    def __init__(
        ...
        if encoder is None:
            ...
            h, w = self.grid_shape[1], self.grid_shape[2]
            for i in range(int(n_encoder)):
                in_channels = self.grid_shape[0] if i == 0 else self.ts_feature[0]
                self.encoder.append(
                    nn.Sequential(
                        nn.Conv2d(in_channels, self.ts_feature[0], kernel_size=kernel_size, stride=kernel_size),
                        nn.SiLU(),
                    )
                )
                h = h // kernel_size
        ...
        self.input_projection = nn.Sequential(nn.Linear(self.n_feature, d_model), nn.Dropout(0.2))
        self.decoder, out_dim = build_mamba_decoder(
            self.ts_feature[0], d_model, n_decoder, use_time_embedding=use_time_embedding
        )
